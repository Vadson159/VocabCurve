  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      data.texts.forEach(text => {
        if (i === 0) {
          point[`${text.id}_cumulative`] = 0;
          point[`${text.id}_new`] = 0;
          return;
        }

        // Map percentage to section index (0 to sections-1)
        const targetSection = Math.min(text.sections - 1, Math.floor((i / 100) * text.sections));
          ...
          >
            ...
            {data.texts.map((text, i) => (
              <Line
                ...
                strokeWidth={3}
                dot={false}
                activeDot={{ r: 6, fill: palette[i % palette.length] }}
              />
            ))}
          </LineChart>
        </ResponsiveContainer>
