import { getStopWords } from "./stop-words.js";
...
export function tokenize(text: string, options: TokenizeOptions): TokenizeResult {
  const expectedScript = options.language === 'ru' ? 'Cyrillic' : 'Latin';
  ...
  const allWords = text
    ...
    .filter((t) => {
      ...
      if (expectedScript === 'Cyrillic') {
        return !/\p{Script=Latin}/u.test(t);
      } else {
        return !/\p{Script=Cyrillic}/u.test(t);
      }
    });
  ...
  const tokens = allWords
    ...
    .filter((t) => !options.filterStopWords || !getStopWords(options.language).has(t));
