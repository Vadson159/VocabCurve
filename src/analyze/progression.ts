...
export function buildFrequencyDistribution(
  ...
  const maxCount = Math.max(...vocabulary.map((w) => w.totalCount), 0);
  const baseThresholds = [1, 2, 3, 5, 10, 20, 50];
  const thresholds = baseThresholds.filter((t) => t <= maxCount || t === 1);

  if (thresholds[thresholds.length - 1] === 50) {
    let next = 100;
    while (next <= maxCount) {
